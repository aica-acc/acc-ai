# -*- coding: utf-8 -*-
"""
app/service/subway_platform/make_screendoor_a_type_high.py

지하철 스크린도어 A형 High(19:9) 외부 광고용 Seedream 입력/프롬프트 생성
...
- 참고용 포스터 이미지(URL 또는 로컬 파일 경로)와 축제 정보(한글)를 입력받아서
  ...
  3) 한글 자리수에 맞춘 플레이스홀더 텍스트(라틴 알파벳 시퀀스)를 사용해서
     19:9 비율 스크린도어 A형 High 프롬프트를 조립한다. (write_screendoor_a_type_high)
  4) 해당 JSON을 받아 Replicate(Seedream)를 호출해 실제 이미지를 생성하고 저장한다. (create_screendoor_a_type_high)
  5) 완성된 배너 이미지를 기반으로 폰트/색상 추천을 수행한다.
  6) run_screendoor_a_type_high_to_editor(...) 로 run_id 기준 editor 폴더에 JSON/이미지 사본을 저장한다.
  7) python make_screendoor_a_type_high.py 로 단독 실행할 수 있다.

전제 환경변수
- OPENAI_API_KEY                           : OpenAI API 키
- BANNER_LLM_MODEL                         : (선택) 기본값 "gpt-4o-mini"
- SUBWAY_SCREENDOOR_A_TYPE_HIGH_MODEL      : (선택) 기본값 "bytedance/seedream-4"
- SUBWAY_SCREENDOOR_A_TYPE_HIGH_SAVE_DIR   : (선택, create_* 단독 사용 시)
    * 절대경로면 그대로 사용
    * 상대경로면 acc-ai 프로젝트 루트 기준
    * 미설정 시 PROJECT_ROOT/app/data/subway_screendoor_a_type_high 사용

CLI 실행:
    python make_screendoor_a_type_high.py
"""
...
# -------------------------------------------------------------
# 1) 영어 씬 묘사 + 플레이스홀더 텍스트 → 스크린도어 A형 High 프롬프트
# -------------------------------------------------------------
def _build_screendoor_a_type_high_prompt_en(
    name_text: str,
    period_text: str,
    location_text: str,
    base_scene_en: str,
    ...
    """
    19:9 비율 지하철 스크린도어 A형 High용 영어 프롬프트 생성.
    ...
    prompt = (
        f"Extra-wide 19:9 rectangular festival illustration of {base_scene_en}, "
        "designed as a high-type wall screen door advertisement in a subway platform, "
...
# -------------------------------------------------------------
# 2) write_screendoor_a_type_high: Seedream 입력 JSON 생성 (+ 플레이스홀더 포함)
# -------------------------------------------------------------
def write_screendoor_a_type_high(
    ...
    """
    지하철 스크린도어 A형 High(19:9, 2161x1024) Seedream 입력 JSON을 생성한다.
    ...
    # 4) 최종 프롬프트 조립 (19:9 스크린도어 A형 High)
    prompt = _build_screendoor_a_type_high_prompt_en(
    ...
    #   - 19:9 비율: width=2161, height=1024
...
# -------------------------------------------------------------
# 3) 스크린도어 A형 High 저장 디렉터리 결정
# -------------------------------------------------------------
def _get_screendoor_a_type_high_save_dir() -> Path:
    """
    SUBWAY_SCREENDOOR_A_TYPE_HIGH_SAVE_DIR 환경변수가 있으면:
      - 절대경로면 그대로 사용
      - 상대경로면 PROJECT_ROOT 기준으로 사용
    없으면:
      - PROJECT_ROOT/app/data/subway_screendoor_a_type_high 사용

    run_screendoor_a_type_high_to_editor(...) 에서는 이 경로 대신
    editor/<run_id>/before_image 를 save_dir 로 직접 넘긴다.
    """
    env_dir = os.getenv("SUBWAY_SCREENDOOR_A_TYPE_HIGH_SAVE_DIR")
    if env_dir:
        p = Path(env_dir)
        if not p.is_absolute():
            p = PROJECT_ROOT / p
        return p
    return DATA_ROOT / "subway_screendoor_a_type_high"


# -------------------------------------------------------------
# 4) create_screendoor_a_type_high: Seedream JSON → Replicate 호출 → 이미지 저장
#     + 플레이스홀더까지 같이 반환
# -------------------------------------------------------------
def create_screendoor_a_type_high(
    seedream_input: Dict[str, Any],
    save_dir: Path | None = None,
) -> Dict[str, Any]:
    """
    write_screendoor_a_type_high(...) 에서 만든 Seedream 입력 JSON을 그대로 받아
    ...
    3) Replicate(bytedance/seedream-4)에 prompt + image_input과 함께 전달해
       실제 19:9 비율 스크린도어 A형 High 이미지를 생성하고,
    4) 생성된 이미지를 로컬에 저장한다.

    save_dir 가 주어지면 해당 디렉터리에 바로 저장하고,
    None 이면 SUBWAY_SCREENDOOR_A_TYPE_HIGH_SAVE_DIR /
    subway_screendoor_a_type_high 기본 경로를 사용한다.
    """
    ...
    width = int(seedream_input.get("width", 2161))
    ...
    model_name = os.getenv(
        "SUBWAY_SCREENDOOR_A_TYPE_HIGH_MODEL",
    ...
    for attempt in range(3):  # 최대 3번까지 시도
        try:
            ...
            raise RuntimeError(
                f"Seedream model error during screendoor_a_type_high generation: {e}"
            )
        except Exception as e:
            # 네트워크 등 다른 예외는 바로 실패
            raise RuntimeError(
                f"Unexpected error during screendoor_a_type_high generation: {e}"
            )

    # 3번 모두 실패한 경우
    if output is None:
        raise RuntimeError(
            f"Seedream model error during screendoor_a_type_high generation after retries: {last_err}"
        )
    ...
    if save_dir is not None:
        ...
        save_base = _get_screendoor_a_type_high_save_dir()
    save_base.mkdir(parents=True, exist_ok=True)

    image_path, image_filename = _save_image_from_file_output(
        file_output, save_base, prefix="screendoor_a_type_high_"
    )
...
# -------------------------------------------------------------
def run_screendoor_a_type_high_to_editor(
    run_id: int,
    poster_image_url: str,
    festival_name_ko: str,
    festival_period_ko: str,
    ...
    동작:
      1) write_screendoor_a_type_high(...) 로 seedream_input 생성
      2) editor/<run_id>/before_data, before_image 디렉터리 생성
      3) create_screendoor_a_type_high(..., save_dir=before_image_dir) 로
         실제 이미지를 생성하고, 곧바로
    ...
    # 1) Seedream 입력 생성
    seedream_input = write_screendoor_a_type_high(
        poster_image_url=poster_image_url,
        festival_name_ko=festival_name_ko,
        festival_period_ko=festival_period_ko,
        festival_location_ko=festival_location_ko,
    ...
    create_result = create_screendoor_a_type_high(
        seedream_input,
    ...
    if image_filename:
        ...
        json_name = f"screendoor_a_type_high_{ts}.json"
...
def main() -> None:
    ...
    # 3) 실제 실행
    result = run_screendoor_a_type_high_to_editor(
        run_id=run_id,
        poster_image_url=poster_image_url,
        festival_name_ko=festival_name_ko,
        festival_period_ko=festival_period_ko,
    ...
    print("✅ screendoor_a_type_high 생성 + 폰트/색상 추천 + editor 저장 완료")
