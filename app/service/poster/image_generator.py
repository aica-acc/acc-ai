...
def translate_to_english(raw_prompt, title_k, date_k, location_k):
    ...
    if text_parts:
        ...
        final_prompt = f"중요: 영어는 절대 사용하지 말고 한글만 사용하세요. {raw_prompt}. 한글 타이포그래피를 포스터의 핵심 디자인 요소로 만들어주세요: {text_instruction}. 글자는 창의적이고 예술적으로 배치하고, 폰트 스타일은 축제 분위기와 완벽하게 조화를 이루며, 입체감과 장식 효과를 추가하세요. 텍스트가 이미지와 자연스럽게 통합되어 하나의 예술작품처럼 보여야 합니다. 텍스트는 전체 포스터의 15-20% 크기로 배치하세요. 고품질, 상세함, 8K, 전문적인 축제 포스터 디자인."
    else:
