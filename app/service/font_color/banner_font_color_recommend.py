...
def _image_path_to_data_url(image_path: str) -> str:
    """
    로컬 이미지 파일 경로를 base64 data URL 로 변환한다.
    """
...
def _safe_font_choice(raw: Any, fallback: str = "Pretendard") -> str:
    """
    LLM이 반환한 font-family 가 우리가 정의한 후보 목록에 있는지 확인.
    없으면 fallback 사용.
    """
    if not isinstance(raw, str):
        return fallback
...
# -------------------------------------------------------------
# 메인: 배너 이미지 + 텍스트 정보 → font + color 추천
# -------------------------------------------------------------
def recommend_fonts_and_colors_for_banner(
    banner_type: str,
    image_path: str,
    ...
    # ---------- System Prompt ----------
    system_prompt = (
        "You are a Korean outdoor festival banner design assistant.\n"
        ...
        "- Do NOT always recommend the same 2 or 3 font families. Actively use diverse font families from the list,\n"
        "  choosing those that best match the mood, color palette, and style of THIS specific banner.\n"
    )

    # ---------- User Prompt (텍스트 + 이미지 동시 전달) ----------
    ...
    user_text = (
        "You will see the final generated festival banner image and metadata.\n"
        ...
        "- It is perfectly fine to choose different font families for the title, period, and location.\n\n"
    ...
    color_period = _normalize_hex_color(
        data.get("festival_color_period_placeholder"), default="#FFFFFF"
    )
