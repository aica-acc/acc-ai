...
- GENERAL_BUS_DRIVEWAY_SAVE_DIR      : (선택, create_* 단독 사용 시)
    * 절대경로면 그대로 사용
...
def _get_general_bus_driveway_save_dir() -> Path:
    ...
    run_general_bus_driveway_to_editor(...) 에서는 이 경로를 사용하지 않고,
    별도로 editor/<run_id>/before_image 에 바로 저장한다.
    """
...
#     + 플레이스홀더까지 같이 반환
# -------------------------------------------------------------
def create_general_bus_driveway(
    seedream_input: Dict[str, Any],
    ...
    None 이면 GENERAL_BUS_DRIVEWAY_SAVE_DIR / general_bus_driveway 기본 경로를 사용한다.
    """
...
def run_general_bus_driveway_to_editor(
    ...
    동작:
      ...
      3) create_general_bus_driveway(..., save_dir=before_image_dir) 로
         실제 배너 이미지를 생성하고, 곧바로
    ...
    create_result = create_general_bus_driveway(
        seedream_input,
    ...
    if image_filename:
        stem = Path(image_filename).stem  # general_bus_driveway_... → general_bus_driveway_....json
