# -*- coding: utf-8 -*-
"""
app/service/bus/make_medium_bus_driveway.py

중형버스 차도면(3:1) 외부 광고용 Seedream 입력/프롬프트 생성
...
- 참고용 포스터 이미지(URL 또는 로컬 파일 경로)와 축제 정보(한글)를 입력받아서
  ...
  3) 한글 자리수에 맞춘 플레이스홀더 텍스트(라틴 알파벳 시퀀스)를 사용해서
     3:1 중형버스 차도면 프롬프트를 조립한다. (write_medium_bus_driveway)
  4) 해당 JSON을 받아 Replicate(Seedream)를 호출해 실제 이미지를 생성하고 저장한다. (create_medium_bus_driveway)
  5) 완성된 배너 이미지를 기반으로 폰트/색상 추천을 수행한다.
  6) run_medium_bus_driveway_to_editor(...) 로 run_id 기준 editor 폴더에 JSON/이미지 사본을 저장한다.
  7) python make_medium_bus_driveway.py 로 단독 실행할 수 있다.
...
CLI 실행:
    python make_medium_bus_driveway.py
"""
...
# -------------------------------------------------------------
# 1) 영어 씬 묘사 + 플레이스홀더 텍스트 → 중형버스 차도면 프롬프트
# -------------------------------------------------------------
def _build_medium_bus_driveway_prompt_en(
    name_text: str,
    period_text: str,
    location_text: str,
    base_scene_en: str,
    ...
    prompt = (
        ...
        "designed to be printed as a long side banner on a medium-size bus, "
...
# -------------------------------------------------------------
# 2) write_medium_bus_driveway: Seedream 입력 JSON 생성 (+ 플레이스홀더 포함)
# -------------------------------------------------------------
def write_medium_bus_driveway(
    ...
    """
    중형버스 차도면(3:1, 3072x1024) Seedream 입력 JSON을 생성한다.
    ...
    # 4) 최종 프롬프트 조립 (3:1 중형버스 차도면)
    prompt = _build_medium_bus_driveway_prompt_en(
...
# -------------------------------------------------------------
# 3) 중형버스 차도면 저장 디렉터리 결정
# -------------------------------------------------------------
def _get_medium_bus_driveway_save_dir() -> Path:
    """
    MEDIUM_BUS_DRIVEWAY_SAVE_DIR 환경변수가 있으면:
      - 절대경로면 그대로 사용
    ...
    if env_dir:
        ...
        if not p.is_absolute():
            p = PROJECT_ROOT / p
        return p
    return DATA_ROOT / "medium_bus_driveway"
...
def create_medium_bus_driveway(seedream_input: Dict[str, Any]) -> Dict[str, Any]:
    ...
    width = int(seedream_input.get("width", 3072))
    ...
    model_name = os.getenv("MEDIUM_BUS_DRIVEWAY_MODEL", "bytedance/seedream-4")
    ...
    for attempt in range(3):  # 최대 3번까지 시도
        try:
            ...
            raise RuntimeError(
                f"Seedream model error during medium bus driveway banner generation: {e}"
            )
        except Exception as e:
            # 네트워크 등 다른 예외는 바로 실패
            raise RuntimeError(
                f"Unexpected error during medium bus driveway banner generation: {e}"
            )
...
def run_medium_bus_driveway_to_editor(
    ...
    # 1) Seedream 입력 생성
    seedream_input = write_medium_bus_driveway(
        poster_image_url=poster_image_url,
        festival_name_ko=festival_name_ko,
        festival_period_ko=festival_period_ko,
        festival_location_ko=festival_location_ko,
    ...
    if image_filename:
        ...
        json_name = f"medium_bus_driveway_{ts}.json"
...
def main() -> None:
    ...
    # 3) 실제 실행
    result = run_medium_bus_driveway_to_editor(
        run_id=run_id,
        poster_image_url=poster_image_url,
        festival_name_ko=festival_name_ko,
        festival_period_ko=festival_period_ko,
    ...
    print("✅ medium bus driveway 배너 생성 + 폰트/색상 추천 + editor 저장 완료")
