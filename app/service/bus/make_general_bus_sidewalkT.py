# -*- coding: utf-8 -*-
"""
app/service/bus/make_general_bus_sidewalkT.py

일반버스 인도면T(18:13) 외부 광고용 Seedream 입력/프롬프트 생성
...
- 참고용 포스터 이미지(URL 또는 로컬 파일 경로)와 축제 정보(한글)를 입력받아서
  ...
  3) 한글 자리수에 맞춘 플레이스홀더 텍스트(라틴 알파벳 시퀀스)를 사용해서
     18:13 비율의 일반버스 인도면T 프롬프트를 조립한다. (write_general_bus_sidewalkT)
  4) 해당 JSON을 받아 Replicate(Seedream)를 호출해 실제 이미지를 생성하고 저장한다. (create_general_bus_sidewalkT)
  5) 완성된 배너 이미지를 기반으로 폰트/색상 추천을 수행한다.
  6) run_general_bus_sidewalkT_to_editor(...) 로 run_id 기준 editor 폴더에 JSON/이미지 사본을 저장한다.
  7) python make_general_bus_sidewalkT.py 로 단독 실행할 수 있다.

전제 환경변수
- OPENAI_API_KEY                       : OpenAI API 키
- BANNER_LLM_MODEL                     : (선택) 기본값 "gpt-4o-mini"
- GENERAL_BUS_SIDEWALKT_MODEL          : (선택) 기본값 "bytedance/seedream-4"
- GENERAL_BUS_SIDEWALKT_SAVE_DIR       : (선택)
    * 절대경로면 그대로 사용
    * 상대경로면 acc-ai 프로젝트 루트 기준
    * 미설정 시 PROJECT_ROOT/app/data/general_bus_sidewalkT 사용

CLI 실행:
    python make_general_bus_sidewalkT.py
"""
...
# -------------------------------------------------------------
# 1) 영어 씬 묘사 + 플레이스홀더 텍스트 → 일반버스 인도면T 프롬프트
# -------------------------------------------------------------
def _build_general_bus_sidewalkT_prompt_en(
    name_text: str,
    period_text: str,
    location_text: str,
    base_scene_en: str,
    ...
    """
    18:13 비율 일반버스 인도면T용 영어 프롬프트 생성.
    인도측 T형 배치에 맞는 메인 템플릿 느낌으로 설계.
    """
    ...
    prompt = (
        f"Slightly wide near-rectangular festival illustration of {base_scene_en}, "
        "designed as the main sidewalk-side T-shaped template for a full-size city bus, "
...
# -------------------------------------------------------------
# 2) write_general_bus_sidewalkT: Seedream 입력 JSON 생성 (+ 플레이스홀더 포함)
# -------------------------------------------------------------
def write_general_bus_sidewalkT(
    ...
    """
    일반버스 인도면T(18:13, 2076x1500) Seedream 입력 JSON을 생성한다.
    ...
    # 4) 최종 프롬프트 조립 (18:13 일반버스 인도면T)
    prompt = _build_general_bus_sidewalkT_prompt_en(
    ...
    #   - 18:13 비율: width=2076, height=1500
...
# -------------------------------------------------------------
# 3) 일반버스 인도면T 저장 디렉터리 결정
# -------------------------------------------------------------
def _get_general_bus_sidewalkT_save_dir() -> Path:
    """
    GENERAL_BUS_SIDEWALKT_SAVE_DIR 환경변수가 있으면:
      - 절대경로면 그대로 사용
      - 상대경로면 PROJECT_ROOT 기준으로 사용
    없으면:
      - PROJECT_ROOT/app/data/general_bus_sidewalkT 사용
    """
    env_dir = os.getenv("GENERAL_BUS_SIDEWALKT_SAVE_DIR")
    if env_dir:
        p = Path(env_dir)
        if not p.is_absolute():
            p = PROJECT_ROOT / p
        return p
    return DATA_ROOT / "general_bus_sidewalkT"


# -------------------------------------------------------------
# 4) create_general_bus_sidewalkT: Seedream JSON → Replicate 호출 → 이미지 저장
#     + 플레이스홀더까지 같이 반환
# -------------------------------------------------------------
def create_general_bus_sidewalkT(seedream_input: Dict[str, Any]) -> Dict[str, Any]:
    """
    write_general_bus_sidewalkT(...) 에서 만든 Seedream 입력 JSON을 그대로 받아
    ...
    3) Replicate(bytedance/seedream-4)에 prompt + image_input과 함께 전달해
       실제 18:13 비율의 일반버스 인도면T 이미지를 생성하고,
    4) 생성된 이미지를 로컬에 저장한다.
    ...
    width = int(seedream_input.get("width", 2076))
    height = int(seedream_input.get("height", 1500))
    ...
    model_name = os.getenv("GENERAL_BUS_SIDEWALKT_MODEL", "bytedance/seedream-4")
    ...
    for attempt in range(3):  # 최대 3번까지 시도
        try:
            ...
            raise RuntimeError(
                f"Seedream model error during general_bus_sidewalkT banner generation: {e}"
            )
        except Exception as e:
            # 네트워크 등 다른 예외는 바로 실패
            raise RuntimeError(
                f"Unexpected error during general_bus_sidewalkT banner generation: {e}"
            )

    # 3번 모두 실패한 경우
    if output is None:
        raise RuntimeError(
            f"Seedream model error during general_bus_sidewalkT banner generation after retries: {last_err}"
        )
    ...
    # 기본 저장 위치: PROJECT_ROOT/app/data/general_bus_sidewalkT
    save_base = _get_general_bus_sidewalkT_save_dir()
    image_path, image_filename = _save_image_from_file_output(
        file_output, save_base, prefix="general_bus_sidewalkT_"
    )
...
# -------------------------------------------------------------
def run_general_bus_sidewalkT_to_editor(
    run_id: int,
    poster_image_url: str,
    festival_name_ko: str,
    festival_period_ko: str,
    ...
    동작:
      1) write_general_bus_sidewalkT(...) 로 seedream_input 생성
      2) create_general_bus_sidewalkT(...) 로 실제 배너 이미지 생성
    ...
    # 1) Seedream 입력 생성
    seedream_input = write_general_bus_sidewalkT(
        poster_image_url=poster_image_url,
        festival_name_ko=festival_name_ko,
        festival_period_ko=festival_period_ko,
        festival_location_ko=festival_location_ko,
    ...
    # 2) 실제 배너 이미지 생성
    create_result = create_general_bus_sidewalkT(seedream_input)

    # 3) 폰트/색상 추천 (bus_type 으로 general_bus_sidewalkT 전달)
    font_color_result = recommend_fonts_and_colors_for_bus(
        bus_type="general_bus_sidewalkT",
    ...
    if original_image_path:
        ...
        if src_image.exists():
            ...
            try:
                # 1순위: general_bus_sidewalkT → editor/before_image 로 이동
    ...
    if image_filename:
        stem = Path(image_filename).stem  # general_bus_sidewalkT → general_bus_sidewalkT.json
        json_name = f"{stem}.json"
    else:
        ts = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
        json_name = f"general_bus_sidewalkT_{ts}.json"
...
def main() -> None:
    ...
    # 3) 실제 실행
    result = run_general_bus_sidewalkT_to_editor(
        run_id=run_id,
        poster_image_url=poster_image_url,
        festival_name_ko=festival_name_ko,
        festival_period_ko=festival_period_ko,
    ...
    print("✅ general_bus_sidewalkT 배너 생성 + 폰트/색상 추천 + editor 저장 완료")
