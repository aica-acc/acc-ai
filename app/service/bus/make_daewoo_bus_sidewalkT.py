...
- 참고용 포스터 이미지(URL 또는 로컬 파일 경로)와 축제 정보(한글)를 입력받아서
  ...
  3) 한글 자리수에 맞춘 플레이스홀더 텍스트(라틴 알파벳 시퀀스)를 사용해서
     19:20 비율의 대우버스 인도면T 프롬프트를 조립한다. (write_daewoo_bus_sidewalkT)
  4) 해당 JSON을 받아 Replicate(Seedream)를 호출해 실제 이미지를 생성하고 저장한다. (create_daewoo_bus_sidewalkT)
  5) 완성된 배너 이미지를 기반으로 폰트/색상 추천을 수행한다.
  6) run_daewoo_bus_sidewalkT_to_editor(...) 로 run_id 기준 editor 폴더에 JSON/이미지 사본을 저장한다.
  7) python make_daewoo_bus_sidewalkT.py 로 단독 실행할 수 있다.
...
CLI 실행:
    python make_daewoo_bus_sidewalkT.py
"""
...
# -------------------------------------------------------------
# 1) 영어 씬 묘사 + 플레이스홀더 텍스트 → 대우버스 인도면T 프롬프트
# -------------------------------------------------------------
def _build_daewoo_bus_sidewalkT_prompt_en(
    name_text: str,
    period_text: str,
    location_text: str,
    base_scene_en: str,
    ...
    """
    19:20 비율 대우버스 인도면T용 영어 프롬프트 생성.
    대우 시내버스 인도측 T형 광고판에 맞는 템플릿 느낌으로 설계.
    """
    ...
    prompt = (
        f"Near-square festival illustration of {base_scene_en}, "
        "designed as the main sidewalk-side T-shaped advertisement template for a Daewoo city bus, "
...
# -------------------------------------------------------------
# 2) write_daewoo_bus_sidewalkT: Seedream 입력 JSON 생성 (+ 플레이스홀더 포함)
# -------------------------------------------------------------
def write_daewoo_bus_sidewalkT(
    ...
    # 4) 최종 프롬프트 조립 (19:20 대우버스 인도면T)
    prompt = _build_daewoo_bus_sidewalkT_prompt_en(
...
# -------------------------------------------------------------
# 3) 대우버스 인도면T 저장 디렉터리 결정
# -------------------------------------------------------------
def _get_daewoo_bus_sidewalkT_save_dir() -> Path:
    """
    DAEWOO_BUS_SIDEWALKT_SAVE_DIR 환경변수가 있으면:
      - 절대경로면 그대로 사용
    ...
    if env_dir:
        ...
        if not p.is_absolute():
            p = PROJECT_ROOT / p
        return p
    return DATA_ROOT / "daewoo_bus_sidewalkT"
...
def create_daewoo_bus_sidewalkT(
    ...
    width = int(seedream_input.get("width", 1920))
    height = int(seedream_input.get("height", 2021))
    ...
    model_name = os.getenv("DAEWOO_BUS_SIDEWALKT_MODEL", "bytedance/seedream-4")
    ...
    for attempt in range(3):  # 최대 3번까지 시도
        try:
            ...
            raise RuntimeError(
                f"Seedream model error during daewoo_bus_sidewalkT banner generation: {e}"
            )
        except Exception as e:
            # 네트워크 등 다른 예외는 바로 실패
            raise RuntimeError(
                f"Unexpected error during daewoo_bus_sidewalkT banner generation: {e}"
            )

    # 3번 모두 실패한 경우
    if output is None:
        raise RuntimeError(
            f"Seedream model error during daewoo_bus_sidewalkT banner generation after retries: {last_err}"
        )
...
# -------------------------------------------------------------
def run_daewoo_bus_sidewalkT_to_editor(
    run_id: int,
    poster_image_url: str,
    festival_name_ko: str,
    festival_period_ko: str,
    ...
    # 1) Seedream 입력 생성
    seedream_input = write_daewoo_bus_sidewalkT(
        poster_image_url=poster_image_url,
        festival_name_ko=festival_name_ko,
        festival_period_ko=festival_period_ko,
        festival_location_ko=festival_location_ko,
    ...
    if image_filename:
        ...
        json_name = f"daewoo_bus_sidewalkT_{ts}.json"
...
def main() -> None:
    ...
    # 3) 실제 실행
    result = run_daewoo_bus_sidewalkT_to_editor(
        run_id=run_id,
        poster_image_url=poster_image_url,
        festival_name_ko=festival_name_ko,
        festival_period_ko=festival_period_ko,
    ...
    print("✅ daewoo_bus_sidewalkT 배너 생성 + 폰트/색상 추천 + editor 저장 완료")
