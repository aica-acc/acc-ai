...
- HYUNDAI_BUS_SIDEWALKT_SAVE_DIR         : (선택)
    * 절대경로면 그대로 사용
...
#     + 플레이스홀더까지 같이 반환
# -------------------------------------------------------------
def create_hyundai_bus_sidewalkT(seedream_input: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    # 기본 저장 위치: PROJECT_ROOT/app/data/hyundai_bus_sidewalkT
...
def run_hyundai_bus_sidewalkT_to_editor(
    ...
    동작:
      ...
      2) create_hyundai_bus_sidewalkT(...) 로 실제 배너 이미지 생성
    ...
    # 2) 실제 배너 이미지 생성
    create_result = create_hyundai_bus_sidewalkT(seedream_input)

    # 3) 폰트/색상 추천 (bus_type 으로 hyundai_bus_sidewalkT 전달)
    ...
    if original_image_path:
        ...
        if src_image.exists():
            ...
            try:
                # 1순위: hyundai_bus_sidewalkT → editor/before_image 로 이동
    ...
    if image_filename:
        stem = Path(image_filename).stem  # hyundai_bus_sidewalkT → hyundai_bus_sidewalkT.json
