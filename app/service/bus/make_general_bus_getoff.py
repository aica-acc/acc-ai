...
- GENERAL_BUS_GETOFF_SAVE_DIR      : (선택, create_* 단독 사용 시)
    * 절대경로면 그대로 사용
...
def _get_general_bus_getoff_save_dir() -> Path:
    ...
    run_general_bus_getoff_to_editor(...) 에서는 이 경로 대신
...
def create_general_bus_getoff(
    ...
    None 이면 GENERAL_BUS_GETOFF_SAVE_DIR / general_bus_getoff 기본 경로를 사용한다.
    """
...
def run_general_bus_getoff_to_editor(
    ...
    동작:
      ...
      3) create_general_bus_getoff(..., save_dir=before_image_dir) 로
         실제 배너 이미지를 생성하고, 곧바로
    ...
    create_result = create_general_bus_getoff(
        seedream_input,
        save_dir=before_image_dir,
    )

    # 4) 폰트/색상 추천 (bus_type 으로 general_bus_getoff 전달)
    ...
    if image_filename:
        stem = Path(image_filename).stem  # general_bus_getoff_... → general_bus_getoff_....json
