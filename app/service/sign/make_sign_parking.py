...
- 참고용 마스코트 이미지(URL 또는 로컬 파일 경로)와 축제 정보(한글)를 입력받아서
  ...
  5) run_sign_parking_to_editor(...) 로 p_no 기준
...
def _log_progress(message: str) -> None:
    """
    주차장 표지판 생성용 진행 로그를 콘솔에 출력한다.
    """
    print(f"[sign_parking] {message}", flush=True)
...
def write_sign_parking(
    ...
    _log_progress("1) 주차장 표지판 Seedream 입력 생성 시작...")
    ...
    # 4) 최종 프롬프트 조립
    _log_progress("4) 주차장 입간판용 프롬프트 조립 중...")
...
def create_sign_parking(
    ...
    _log_progress("6) Seedream 모델 호출 및 주차장 표지판 이미지 생성 단계 진입...")
    ...
    _log_progress(f"✔ 주차장 표지판 이미지 저장 완료: {final_path}")
...
def run_sign_parking_to_editor(
    ...
    반환:
      {
        "db_file_type": "sign_parking",
        "type": "image",
        "db_file_path": "<FRONT_PROJECT_ROOT>\\public\\data\\promotion\\M000001\\{p_no}\\sign\\sign_parking.png",
        "type_ko": "주차장 표지판"
      }
    """

    _log_progress("==============================================")
    _log_progress("▶ 주차장 표지판 생성(run_sign_parking_to_editor) 시작")
    ...
    # 3) 이미지 생성
    _log_progress(
        "▶ 3단계: Seedream 모델 호출 및 주차장 표지판 이미지 생성 시작 (시간이 조금 걸릴 수 있습니다)..."
    )
    ...
    _log_progress("✔ 주차장 표지판 생성 완료. DB 메타 정보 리턴.")
    _log_progress("==============================================")
