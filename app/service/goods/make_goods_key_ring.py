...
- 참고용 마스코트 이미지(URL 또는 로컬 파일 경로)와 축제 정보(한글)를 입력받아서
  ...
  5) run_goods_key_ring_to_editor(...) 로 p_no 기준
...
def _log_progress(message: str) -> None:
    """
    키링 굿즈용 진행 로그를 콘솔에 즉시 출력한다.
    (flush=True 로 버퍼링 없이 실시간 표시)
    """
    print(f"[goods_key_ring] {message}", flush=True)
...
def write_goods_key_ring(
    ...
    """
    _log_progress("1) 축제 정보 번역 및 프롬프트용 데이터 구성 시작...")
    ...
    _log_progress(
        f"   - 축제 영문 변환 완료: name_en='{name_en}', period_en='{period_en}', location_en='{location_en}'"
    )
    ...
    _log_progress(
        f"   - 씬 분석 결과: base_scene_en='{scene_info['base_scene_en'][:60]}...', "
        f"details_phrase_en='{scene_info['details_phrase_en'][:60]}...'"
    )

    # 4) 최종 프롬프트 조립
    _log_progress("3) 키링 세트 프롬프트 조립 중...")
...
def create_goods_key_ring(
    ...
    _log_progress("4) Seedream 모델 호출 및 이미지 생성 단계 진입...")
    ...
    _log_progress(f"5) 생성 이미지 저장 디렉터리 준비 완료: {save_base}")
    ...
    _log_progress(f"✔ 생성 이미지 저장 완료: {final_path}")
...
def run_goods_key_ring_to_editor(
    ...
    반환:
      {
        "db_file_type": "goods_key_ring",
        "type": "image",
        "db_file_path": "<FRONT_PROJECT_ROOT>\\public\\data\\promotion\\M000001\\{p_no}\\goods\\goods_key_ring.png",
        "type_ko": "키링 굿즈"
      }
    """

    _log_progress("==============================================")
    _log_progress("▶ 키링 굿즈 생성(run_goods_key_ring_to_editor) 시작")
    ...
    _log_progress(f"▶ 2단계: 저장 디렉터리 준비 완료 → {goods_dir}")
    ...
    _log_progress("✔ 키링 굿즈 생성 완료. DB 메타 정보 리턴.")
    _log_progress("==============================================")
...
def main() -> None:
    ...
    mascot_image_url = r"C:\final_project\ACC\acc-ai\app\data\mascot\panda.png"
