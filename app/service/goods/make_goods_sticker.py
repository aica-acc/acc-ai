...
- 참고용 마스코트 이미지(URL 또는 로컬 파일 경로)와 축제 정보(한글)를 입력받아서
  ...
  5) run_goods_sticker_to_editor(...) 로 p_no 기준
...
def _log_progress(message: str) -> None:
    """
    스티커 굿즈용 진행 로그를 콘솔에 즉시 출력한다.
    (flush=True 로 버퍼링 없이 실시간 표시)
    """
    print(f"[goods_sticker] {message}", flush=True)
...
def run_goods_sticker_to_editor(
    ...
    반환:
      {
        "db_file_type": "goods_sticker",
        "type": "image",
        "db_file_path": "<FRONT_PROJECT_ROOT>\\public\\data\\promotion\\M000001\\{p_no}\\goods\\goods_sticker.png",
        "type_ko": "스티커 굿즈"
      }
    """

    _log_progress("==============================================")
    _log_progress("▶ 스티커 굿즈 생성(run_goods_sticker_to_editor) 시작")
    ...
    _log_progress("✔ 스티커 굿즈 생성 완료. DB 메타 정보 리턴.")
    _log_progress("==============================================")
