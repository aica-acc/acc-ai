...
def _log_progress(message: str) -> None:
    """
    이모티콘 굿즈용 진행 로그를 콘솔에 즉시 출력한다.
    (flush=True 로 버퍼링 없이 실시간 표시)
    """
    print(f"[goods_emoticon] {message}", flush=True)
...
def write_goods_emoticon(
    ...
    # 4) 최종 프롬프트 조립
    _log_progress("3) 이모티콘 세트 프롬프트 조립 중...")
...
def run_goods_emoticon_to_editor(
    ...
    _log_progress("==============================================")
    _log_progress("▶ 이모티콘 굿즈 생성(run_goods_emoticon_to_editor) 시작")
    ...
    _log_progress("✔ 이모티콘 굿즈 생성 완료. DB 메타 정보 리턴.")
    _log_progress("==============================================")
