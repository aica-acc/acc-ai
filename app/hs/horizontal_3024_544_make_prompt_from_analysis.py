...
- analysis.json만 보고 Dreamina 3.1용 영문 프롬프트 JSON 생성
- 가로형 3024×544 기본 / aspect_ratio='custom'
...
DEFAULT_AR     = "custom"
DEFAULT_RES    = "2K"
DEFAULT_USE_LLM= True  # ← 요청대로 기본 true
...
def llm_compose_prompt(analysis: dict) -> str:
    ...
    system_msg = (
        ...
        "Write ONE concise English prompt for bytedance/dreamina-3.1 to generate a LONG HORIZONTAL FESTIVAL BANNER BACKGROUND.\n"
        "Rules:\n"
        "- Use only the JSON the user provides; infer season/theme/visuals from it.\n"
        "- Background only; kid-safe; NO people. Bold sans-serif; high contrast; clean composition; wide clear area on the LEFT for headline.\n"
        ...
        "- Use 4–8 vivid visual descriptors from the JSON (keywords/summary/programs/events); keep the whole prompt succinct (<=300 chars ideally).\n"
...
def main():
    print("=== Dreamina 3.1 프롬프트 생성기 (analysis.json -> prompt JSON, 3024x544 기본) ===")
